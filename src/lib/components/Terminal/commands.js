...
export const commands = {
	help: {
		...
		execute: () => {
			const commandList = Object.values(commands)
				.map((cmd) => `  ${colorize(cmd.name.padEnd(12), 'cyan')} - ${cmd.description}`)
				.join('\n');
  ...
  • ${colorize('E-commerce Platform', 'cyan')} - ${colorText('Full-stack SvelteKit application', {
    color: 'blue',
    background: 'white',
    bold: true,
    underline: true
  ...
  ${colorize('📧 Email:', 'cyan')} hello@iamickdev.com
  ${colorize('🐙 GitHub:', 'magenta')} github.com/iamickdev
  ${colorize('💼 LinkedIn:', 'blue')} linkedin.com/in/iamickdev
  ${colorize('🐦 Twitter:', 'cyan')} @iamickdev`
	},
